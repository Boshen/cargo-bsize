...
impl<W: Write> CargoBsize<W> {
    ...
    fn analyze(&mut self) -> Result<()> {
        ...
        let mut report = output::Report { duplicates, ..output::Report::default() };
        ...
        if let Some(target) = build::select_target(
            ...
            self.analyze_target(&metadata, &target, &levers, &mut report)?;
        }
    ...
    /// Build and analyze the selected linked target, adding each available view
    /// to `report`.
    fn analyze_target(
        &self,
        metadata: &Metadata,
        ...
        let types = debug.as_deref().and_then(|debug| types::analyze(debug, workspace, limit).ok());
        let (sites, static_sizes, provenance) = match types {
            Some(types) => {
                ...
                provenance.cost_duplicates(&mut report.duplicates);
                report.types = Some(types.report);
                (types.sites, types.static_sizes, Some(provenance))
            }
            None => (FxHashMap::default(), FxHashMap::default(), None),
        };
        ...
        provenance::attach(report, &sites);

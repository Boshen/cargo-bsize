...
cargo bsize --macros                   # rank macros by the source they expand to
...
`--llvm-ir`, `--mono`, `--macros`, `--remarks`, and `--what-if` perform
additional builds and can be slow (`--mono` reads `-Zdump-mono-stats`,
`--macros` reads `-Zmacro-stats` from a check build in its own cache, and
`--remarks` reads `-Cremark` through `-Zremark-dir`, all via
`RUSTC_BOOTSTRAP=1`). By
...
- inlined code by callee, caller, origin crate, and line; panic and formatting
  overhead, and dynamic dispatch;
- code per workspace file and directory;
...
- optional baseline, LLVM IR, monomorphization cost, macro-expansion cost,
  expanded-loop, and measured what-if comparisons.
